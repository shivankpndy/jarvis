...
client     = mqtt.Client()
_connected = False
_notify_fn = None        # JARVIS speak function
_notif_lock = threading.Lock()   # prevent double notifications
...
def _speak_once(text):
    """Speak a notification — lock prevents overlapping sounds."""
    with _notif_lock:
        if _notify_fn:
            _notify_fn(text)
...
def connect():
    ...
    try:
        ...
        client.loop_start()
        _connected = True
        print("IoT agent connected to MQTT broker")
...
# ── Automatic Tea Scheduler ────────────────────────────────────────
def morning_tea():
    print("Scheduler: Morning tea time!")
    publish("jarvis/tea", "on")   # green LED on for 30s (ESP32 handles auto-off)
    _speak_once("Good morning sir. It is time for your morning tea.")


def evening_tea():
    print("Scheduler: Evening tea time!")
    publish("jarvis/tea", "on")
    _speak_once("Good evening sir. It is time for your evening tea.")


def start_tea_scheduler():
    schedule.every().day.at("06:00").do(morning_tea)
    schedule.every().day.at("17:00").do(evening_tea)

    def _run():
        print("Tea scheduler running — 6AM and 5PM")
        while True:
...
def handle(user_text: str) -> str:
    ...
    print(f"IoT received: '{text}'")

    # ── White LED — room lights (stays on until told off) ──
    ...
    # ── Green LED — tea (30 seconds, ESP32 handles auto-off) ──
    ...
    # ── Red LED — blink only, always off after ──
...
def trigger_alert(alert_type="blink"):
    """Called externally by OpenCV — always just blinks red."""
    publish("jarvis/alert", "blink")
