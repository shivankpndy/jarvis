...
async def generate_jarvisresponse(caller_name, user_message, conversation_history):
    ...
    if isinstance(call, PhoneCallDiscarded):
        if isinstance(getattr(call, 'reason', None), PhoneCallDiscardReasonMissed):
            ...
            if caller_id:
                ...
                await client.send_message(caller_id, autoresponse)
...
async def handle_incoming_message(event):
    ...
    try:
        ...
        reply = await generate_jarvisresponse(caller_name, user_message, conv["messages"][:-1])
