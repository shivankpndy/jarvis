...
import os as _os
_PROVIDER = _os.getenv("LLM_PROVIDER", "cohere").lower()
_COHERE_KEY = _os.getenv("COHERE_API_KEY", "")

if _PROVIDER == "cohere" and _COHERE_KEY:
    llm       = LLM(model="cohere/command-r",    api_key=_COHERE_KEY)
    coder_llm = LLM(model="cohere/command-r",    api_key=_COHERE_KEY)
else:
